...
def lockin_fft(directory_name, parameters, inst_call, feed_str = 'feed_on'):
    ...
    [photons, bias, temp, mod_volt,
     ...
     gate_source_no, gate_voltage_divider, sample_rate_awg,
    ...
    par_array.append([line()-line0,"Delta ng for inducing error signal jump", delta_ng])
    par_array.append([line()-line0,"Pulse frequency of gate",pulse_freq])
    par_array.append([line()-line0,"AWG sample rate", sample_rate_awg])
    ...
    [mod_volt,
     ...
     delta_ng, pulse_freq, sample_rate_awg,
     span, res_bw, video_bw, avg, ref_level, PID_prompt] = scan_parameters
    ...
    data_off_res_0_offset = dig.save_data(acquire_data, clock_rate, 
                                       sample_rate = sample_rate, 
                                       channels = 2, split = False, 
    ...
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    data_off_res_1_offset = dig.save_data(acquire_data, clock_rate, 
                                       sample_rate = sample_rate, 
                                       channels = 2, split = False, 
                                       input_range_V = input_range_V)
    ...
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    data_on_res_0_sig = dig.save_data(acquire_data, clock_rate, 
                                       sample_rate = sample_rate, 
                                       channels = 2, split = False, 
                                       input_range_V = input_range_V)
    ...
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    data_on_res_1_sig = dig.save_data(acquire_data, clock_rate, 
                                       sample_rate = sample_rate, 
                                       channels = 2, split = False, 
                                       input_range_V = input_range_V)
    ...
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    data_off_res_1_sig = dig.save_data(acquire_data, clock_rate, 
                                       sample_rate = sample_rate, 
                                       channels = 2, split = False, 
                                       input_range_V = input_range_V)
    ...
    #Prepare the AWG to send pulse. Calibrate the voltage factor for AWG.
#    ng0_at_sample = gate_meter.get_voltage()
    gate_input_new = gate_fn([bias[1]+delta_ng])[0]
#    gate_add = gate_fn([bias[1]+delta_ng])[0]-gate_fn([bias[1]])[0]
    fn.bias_source_set_volt(gate_source, gate_input_new, 1, 
                            volt_div = gate_voltage_divider, daq = daq)
    time.sleep(1)
    ng_new = gate_meter.get_voltage()
#    gate_att_correct = gate_input_new*gate_voltage_divider/ng_new
    print('Gate measured using daq shift {} mV'.format(ng_new*1e3))
    fn.bias_source_set_volt(gate_source, gate, 1, 
                            volt_div = gate_voltage_divider, daq = daq)
    time.sleep(1)
    period = 1/pulse_freq
    waveform_points = int(period*sample_rate_awg)
    time_array = np.linspace(0, period, waveform_points+1)
    gate_add = gate_fn([bias[1]+delta_ng])[0]-gate_fn([bias[1]])[0]
    volt_array = np.array([gate_add*gate_voltage_divider for 
                           volt in range(waveform_points)])
    
    """PLOT 3"""
    plt.figure()
    plt.xlabel('Time($\mu$s)')
    plt.ylabel('Output (V)')
    plt.plot(time_array[:-1]*1e6, volt_array, lw=4)
    plt.savefig(directory_name_fft+'/dc_waveform_before_correction.jpeg')
    plt.close()
    
    gate_filename = '/Sisira/gate_dc_before.wfm'
    fn.create_waveform_awg(volt_array, sample_rate_awg, gate_filename)
    np.savetxt(directory_name_fft+'/dc_before.dat',
               np.column_stack((time_array[:-1], volt_array)))
    
    awg.toggle_fg_mode(0)
    awg.load_waveform(1, gate_filename)
    awg.toggle_output(1, 1)
    awg.run()
    time.sleep(3)
    
    ng_new = gate_meter.get_voltage()
    print('Gate measured using AWG shift {} mV'.format(ng_new*1e3))
    
    "Measure the quadrature response with AWG set to delta ng DC."
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    data_delta_ng_dc = dig.save_data(acquire_data, clock_rate, 
                                     sample_rate = sample_rate, 
                                     channels = 2, split = False, 
                                     input_range_V = input_range_V)
    filename=(directory_name_fft+'/quadrature_signal_on_resonance_deltang')
    np.savetxt(filename+'.dat', np.column_stack((data_delta_ng_dc[0],
                                                 data_delta_ng_dc[1],
                                                 data_delta_ng_dc[2])),
    encoding = 'utf-16')
    
    halfway_points = int(waveform_points/2)
    volt_array = np.zeros(waveform_points)
#    gate_values = [0, gate_add*gate_att_correct]
    gate_values = [0, gate_add*gate_voltage_divider]
    time_array = np.linspace(0, period, waveform_points+1)
#    volt_array[:halfway_points] = gate_values[0]
#    volt_array[halfway_points:] = gate_values[1]
    volt_array = gate_values[1]*np.sin(2*np.pi*pulse_freq*time_array[:-1])
    ...
    plt.ylabel('Output (V)')
    plt.plot(time_array[:-1]*1e6, volt_array, lw=4)
    plt.savefig(directory_name_fft+'/pulse_waveform.jpeg')
    plt.close()
    
    gate_filename = '/Sisira/gate_pulse.wfm'
    fn.create_waveform_awg(volt_array, sample_rate_awg, gate_filename)
    np.savetxt(directory_name_fft+'/pulse.dat',
               np.column_stack((time_array[:-1], volt_array)))
    awg.load_waveform(1, gate_filename)
    time.sleep(3)
    "Measure the quadrature response with the AWG sending pulse."
    acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
    #    acquire_data = dig.acquire(3,0,64,1,acq_time)
    data_pulse = dig.save_data(acquire_data, clock_rate, 
                               sample_rate = sample_rate, 
                               channels = 2, split = False, 
                               input_range_V = input_range_V)
    filename=(directory_name_fft+'/quadrature_signal_pulse')
    np.savetxt(filename+'.dat', np.column_stack((data_pulse[0],
                                                 data_pulse[1],
                                                 data_pulse[2])),
    encoding = 'utf-16')
    #Measure power spectrum with AWG sending a pulse.
    filename_data=(directory_name_fft+'/fridge_out_signal_pulse.dat')
    (par_array, freq_sig, power_sig) = fn.spec_scan(
            res_freq_meas, 
            span,
            res_bw, video_bw, avg, ref_level, filename_data)
    sig_avg_pulse = avg_data(freq_sig, power_sig)
    ...
    plt.plot(data_pulse[0][:-points_omit],
             data_pulse[1][:-points_omit]/gate_voltage_divider*1e3, 
             label = 'Pulse', lw=4)
    plt.plot(data_on_res_1_sig[0][:-points_omit],
             data_on_res_1_sig[1][:-points_omit]/gate_voltage_divider*1e3, 
             label = 'Bias point = {} e'.format(bias[1]), lw=4)
    plt.plot(data_delta_ng_dc[0][:-points_omit],
             data_delta_ng_dc[1][:-points_omit]/gate_voltage_divider*1e3, 
             label = 'Bias point = {} e'.format(round(bias[1]+delta_ng,3)),
             lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       tc, filt)+
                       ...
                       format(
                       bias[0], bias[1], round(bias[1]+delta_ng,3))+
                               '\n Pulse frequency = {} Hz'.format(
                                       pulse_freq)), fontsize=20, 
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/ng_compare_bias_points'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    plt.ylabel('Lock-in output Y (V)',fontsize = 30)
    plt.plot(data_pulse[0][:-points_omit],
             data_pulse[2][:-points_omit], 
             label = 'Pulse', lw=4)
    plt.plot(data_on_res_1_sig[0][:-points_omit],
             data_on_res_1_sig[2][:-points_omit], 
             label = 'Bias point = {} e'.format(bias[1]), lw=4)
    plt.plot(data_delta_ng_dc[0][:-points_omit],
             data_delta_ng_dc[2][:-points_omit], 
             label = 'Bias point = {} e'.format(round(bias[1]+delta_ng,3)), 
             lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
    ...
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/Y_Quadrature_compare_bias_points'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    if feed_prompt=='y': #prompts to check the input values
        print('Running..')
        time.sleep(3)
        "Measure quadrature response with feedback and pulse on."
        acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
        data_pulse_feed = dig.save_data(acquire_data, clock_rate, 
                                        sample_rate = sample_rate, 
                                        channels = 2, split = False, 
                                        input_range_V = input_range_V)
        filename=(directory_name_fft+'/quadrature_signal_pulse_feed')
        np.savetxt(filename+'.dat', np.column_stack((data_pulse_feed[0],
                                                     data_pulse_feed[1],
                                                     data_pulse_feed[2])),
        encoding = 'utf-16')
        
        filename_data=(directory_name_fft+'/fridge_out_signal_feed_pulse.dat')
        (par_array, freq_sig, power_sig) = fn.spec_scan(
                res_freq_meas, 
                span,
                res_bw, video_bw, avg, ref_level, filename_data)
        sig_avg_feed_pulse = avg_data(freq_sig, power_sig)
        #Switch off pulse.
        awg.toggle_output(1, 0)
        time.sleep(3)
        acquire_data = dig.acquire(3, 0, samples, 1, acq_no)
        data_feed_dc = dig.save_data(acquire_data, clock_rate, 
                                           sample_rate = sample_rate, 
                                           channels = 2, split = False, 
                                           input_range_V = input_range_V)
        filename=(directory_name_fft+'/quadrature_signal_pulse_feed_dc')
        np.savetxt(filename+'.dat', np.column_stack((data_feed_dc[0],
                                                     data_feed_dc[1],
                                                     data_feed_dc[2])),
        encoding = 'utf-16')
        #Measure power spectrum with feedback on but pulse off.
    ...
    """PLOT 7"""
    points_plot = int((sample_rate/pulse_freq)*10)
    if points_plot>len(data_pulse_feed[0]):
        points_plot = len(data_pulse_feed[0])
    ...
    ng_corrected_pulse = (slope_V_ng*data_pulse_feed[1]
    [:points_plot]/gate_voltage_divider+intercept_V_ng)
    ...
    ng_pulse = (slope_V_ng*data_pulse[1]
    [:points_plot]/gate_voltage_divider+intercept_V_ng)
    plt.plot(data_pulse[0][:points_plot],
             ng_pulse, 
             label = 'Pulse', lw=4)
    plt.plot(data_pulse_feed[0][:points_plot], 
             ng_corrected_pulse, 
             label = 'Pulse with feedback on', lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       tc, filt)+
                       ' Photons = {}\n'.format(photons)+
    ...
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/pulsed_ng_with_feedback'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    plt.ylabel('Lock-in output Y (V)',fontsize = 30)
    plt.plot(data_pulse[0][:points_plot],
             data_pulse[2][:points_plot], 
             label = 'Pulse', lw=4)
    plt.plot(data_pulse_feed[0][:points_plot],
             data_pulse_feed[2][:points_plot], 
             label = 'Pulse with feedback on', lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       tc, filt)+
    ...
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/pulsed_Y_Quadrature_with_feedback'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    ngf_delta_ng = fft(data_delta_ng_dc[1]
    [:-points_omit]/gate_voltage_divider*1e3)
    yf_delta_ng = fft(data_delta_ng_dc[2][:-points_omit])
    ngf_pulse = fft(data_pulse[1][:-points_omit]/gate_voltage_divider*1e3)
    yf_pulse = fft(data_pulse[2][:-points_omit])
    ngf_feed = fft(data_pulse_feed[1][:-points_omit]/gate_voltage_divider*1e3)
    yf_feed = fft(data_pulse_feed[2][:-points_omit])
    ...
    valid_deltang_ng = 2/N*np.abs(ngf_delta_ng[:N//2])
    valid_deltang_y = 2/N*np.abs(yf_delta_ng[:N//2])
    valid_pulse_ng = 2/N*np.abs(ngf_pulse[:N//2])
    valid_pulse_y = 2/N*np.abs(yf_pulse[:N//2])
    valid_feed_ng_pulse = 2/N*np.abs(ngf_feed[:N//2])
    valid_feed_y_pulse = 2/N*np.abs(yf_feed[:N//2])
    ...
    plt.plot(tf[1:], valid_off_res_y[1:],
             label = 'off resonance noise', lw=4)
    plt.plot(tf[1:], valid_on_res_y[1:],
             label = 'on resonance noise', lw=4)
    plt.plot(tf[1:], valid_sig_y[1:],
    ...
    ax.tick_params(axis='y',direction='out', labelsize=40,width=2,length=10)
    plt.xlabel('Frequency (Hz)',fontsize = 30)
    plt.ylabel('FFT ng', fontsize = 30)
    plt.plot(tf[1:], valid_pulse_ng[1:],
             label = 'Pulse', lw=4)
    plt.plot(tf[1:], valid_deltang_ng[1:],
             label = 'Bias point = {} e'.format(round(bias[1]+delta_ng,3)), 
             lw=4)
    plt.plot(tf[1:], valid_sig_ng[1:],
             label = 'Bias point = {} e'.format(bias[1]), lw=4)
    plt.xscale('log')
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
    ...
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/ng_FFT_pulse'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    plt.xlabel('Frequency (Hz)',fontsize = 30)
    plt.ylabel('FFT Y', fontsize = 30)
    plt.plot(tf[1:], valid_pulse_y[1:],
             label = 'Pulse', lw=4)
    plt.plot(tf[1:], valid_deltang_y[1:],
             label = 'Bias point = {} e'.format(round(bias[1]+delta_ng,3)), 
             lw=4)
    plt.plot(tf[1:], valid_sig_y[1:],
             label = 'Bias point = {} e'.format(bias[1]), lw=4)
    ...
    y=0.65)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/Y_FFT_pulse'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    ax1.plot(tf[1:], valid_feed_ng_pulse[1:],
             label = 'Feedback on', lw=4)
    ax1.plot(tf[1:], valid_pulse_ng[1:],
             label = 'Feedback off', lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       tc, filt)+
    ...
    filename = directory_name_fft + '/Feedback_FFT_pulsed_ng'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    ax3.set_xscale('log')
    ax3.plot(tf[1:], valid_pulse_y[1:],
             label = 'Feedback off Y', lw=4)
    ax3.plot(tf[1:], valid_feed_y_pulse[1:],
             label = 'Feedback on Y', lw=4)
    ax3.plot(tf[1:], valid_off_res_y_sig[1:],
               ...
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       ...
                       bias[0], bias[1], round(bias[1]+delta_ng,3))+
                               '\n Pulse frequency = {} Hz'.format(
                                       pulse_freq)), fontsize=20)
    plt.legend(fontsize = 20)
    filename = directory_name_fft + '/Feedback_FFT_Y_pulsed'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    ax1.tick_params(axis='x',direction='out', labelsize=40,width=2,length=10)
    ax1.tick_params(axis='y',direction='out', labelsize=40,width=2,length=10)
    ax1.plot(tf[1:], valid_feed_ng_dc[1:],
             label = 'Feedback on', lw=4)
    ax1.plot(tf[1:], valid_sig_ng[1:],
             label = 'Feedback off', lw=4)
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
               'Time constant = {} s, \nFilter slope = {} dB/oct,'.format(
                       tc, filt)+
    ...
    ax3.set_ylabel('FFT Y', fontsize = 30)
    ax3.set_xscale('log')
    ax3.plot(tf[1:], valid_sig_y[1:],
             label = 'Feedback off Y', lw=4)
    ax3.plot(tf[1:], valid_feed_y_dc[1:],
             label = 'Feedback on Y', lw=4)
    ax3.plot(tf[1:], valid_off_res_y_sig[1:],
             label = 'Off resonance', lw=4) 
    plt.title(('Sensitivity = {:E} nV,'.format(sens)+
    ...
    plt.plot(sig_avg_off_res[0][start:], sig_avg_off_res[2][start:],
             label = 'Off resonance', lw =4)
    plt.plot(sig_avg_pulse[0][start:], sig_avg_pulse[2][start:],
             label = 'Feedback off Pulse', lw =4)
    plt.plot(sig_avg[0][start:], sig_avg[2][start:],
             label = 'Beginning noise', lw =4)
    plt.plot(sig_avg_feed_pulse[0][start:], sig_avg_feed_pulse[2][start:],
             label = 'Feedback on Pulse', lw =4)
    ...
    filename=(directory_name_fft+'/output_noise_feedback_pulse.jpeg')
    ...
    np.savetxt(directory_name_fft +'/fft.dat', np.column_stack((
            ...
            valid_deltang_ng, valid_deltang_y,
            valid_pulse_ng, valid_pulse_y,
            valid_feed_ng_pulse, valid_feed_y_pulse)))
    
    return (tf, valid_off_res_ng, valid_off_res_y,
            valid_on_res_ng, valid_on_res_y,
            valid_sig_ng, valid_sig_y,
            valid_deltang_ng, valid_deltang_y,
            valid_pulse_ng, valid_pulse_y,
