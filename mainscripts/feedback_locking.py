...
directory_name=('../{}_{}_{}_{}_feedback_fft_{}_mK_{}_ng_{}_phi0_{}_photons'.
...
par_array.append([line()-line0,"Signal generator carrier power to J0 diff", 19.94])
par_array.append([line()-line0,"Modulation frequency",30e6])
par_array.append([line()-line0,"Modulation voltage (Vpp)",0.31])
par_array.append([line()-line0,"Lockin sensitivity (nV)",300e6])
par_array.append([line()-line0,"Lock-in time constant for sweep (sec)",10e-3])
par_array.append([line()-line0,"Lock-in time constant for feedback (sec)",100e-6])
...
(res_freq, res_freq_meas, res_freq_ref, kappa_int, kappa_ext,
...
vna_scan_parameters = [bias_point, temp,
                       ...
                       flux_fn, gate_fn]
(res_freq, res_freq_meas) = mm.res_scan_twpa_on(directory_name, 
                                                 vna_scan_parameters, 
                                                 scan_no = 3, 
                                                 inst_call = inst_call)
...
ng_e_array = np.linspace(bias[1]-0.05,bias[1]+0.05,11)
essng_params = [photons, bias, temp, mod_freq, mod_volt, res_freq_meas,
                sens, tc_sweep, phase_ref, expand, filt, 
                clock_rate, sample_rate_sweep, acq_no, acq_time_sweep,
                bp_volt_fn, ng_e_array,
                gate_source_no, gate_voltage_divider]
(ng_e_array, y_array, gate_array) = mm.error_signal_sweep_ng(
        directory_name, essng_params, inst_call, feed_str='')

"""Getting zero point"""
zero_pos = np.where(np.abs(y_array)==np.min(np.abs(y_array)))[0][0]
bias[1] = ng_e_array[zero_pos]

"""Get gain"""
gain = np.mean([(y_array[zero_pos+1]-y_array[zero_pos]),
                (y_array[zero_pos]-y_array[zero_pos-1])])/0.01
...
if feed_bool:
    ...
    setup_parameters = [photons, bias, temp, mod_volt, sens, tc_feed, 
                        phase_ref, expand, filt, 
                        clock_rate, sample_rate_setup, acq_no, acq_time_setup, 
                        res_freq_meas, carrier_power, mod_freq, 
                        gate_voltage_divider]
    offset_y = mm.lockin_offset_feed(directory_name, setup_parameters, 
                                     inst_call,
                                     feed_str = '')
    
#"""Response measurement using spectrum analyzer"""
#os.rename(directory_name+'/Output_signal',
#          directory_name+'/Output_signal_initial')
#output_parameters = [carrier_power, res_freq_meas, mod_freq, mod_volt,
#                     bias, flux_fn, gate_fn, gate_source, gate_voltage_divider,
#                     photons]
#mm.output_measure(directory_name, output_parameters, inst_call)
