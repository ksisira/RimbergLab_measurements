...
Acquires the error signal as a function of gate with and without feedback.
...
Steps:
    ...
    10. Continue measurements with feedback.
...
Manually set:
    ...
    7. Connect directional coupler second output to DCOM 
    through power detector.
...
directory_name=('../{}_{}_{}_{}_ng_sweep_feed_{}_mK_{}_ng_{}_phi0_{}_photons'.
...
par_array.append([line()-line0,"Lock-in time constant for sweep (sec)",10e-3])
par_array.append([line()-line0,"Lock-in time constant for feedback (sec)",10e-3])
...
par_array.append([line()-line0,"P",1])
par_array.append([line()-line0,"I",1e2])
par_array.append([line()-line0,"D",1e-5])
par_array.append([line()-line0,"UL",5])
par_array.append([line()-line0,"LL",-3])
...
"""Error signal scan for ng sweep"""
if ng_sweep_state:
    flux = flux_fn([bias[0]])[0]
    fn.bias_source_set_volt('daq', flux, 0, daq = daq)
    essng_params = [photons, bias, temp, mod_freq, mod_volt, res_freq_meas,
                    sens, tc_sweep, phase_ref, expand, filt, 
                    clock_rate, sample_rate_sweep, acq_no, acq_time_sweep,
                    bp_volt_fn, ng_stop, ng_start, 
                    ng_sweep_points, vna_power_dBm, el_delay, smooth_param,
                    gate_source_no, gate_voltage_divider, twpa_state]
    (ng_array, y_array, 
     gate_array, freq_array) = mm.error_signal_sweep_ng_feed(
            directory_name, essng_params, inst_call, feed_str='')

"""Refresh setup"""
setup_parameters = [photons, bias, temp, mod_volt, sens, tc_feed, phase_ref, 
                    expand, filt, 
                    clock_rate, sample_rate_setup, acq_no, acq_time_setup,  
                    res_freq_meas, carrier_power, mod_freq, 
                    gate_voltage_divider]
offset_y = mm.lockin_offset_feed(directory_name, setup_parameters, inst_call,
                                 feed_str='_low_tc_refresh_1')
...
setup_parameters = [photons, bias, temp, mod_volt, sens, tc_feed, phase_ref, 
                    ...
                    gate_voltage_divider]
offset_y = mm.lockin_offset_feed(directory_name, setup_parameters, inst_call,
                                 feed_str='_low_tc_refresh_2')

"""Error signal with feedback on"""
if feed_bool:
    flux = flux_fn([bias[0]])[0]
    fn.bias_source_set_volt('daq', flux, 0, daq = daq)
    essng_params = [photons, bias, temp, mod_freq, mod_volt, res_freq_meas,
    ...
    if feed_prompt=='y': #prompts to check the input values
        print('Running..')
        (ng_array, y_array, gate_array) = mm.error_signal_sweep_ng_feed(
                directory_name, essng_params, inst_call, 
                feed_str = '_feedback_on')
        reset_prompt = input('Reset feedback. (y)')
    else:
        print('No feedback measurement.')        
...
"""Save results file"""
results_file = directory_name+'/results_feedback_sweep.txt'
