...
def lockin_offset(directory_name, parameters, inst_call):
    directory_name_off = directory_name + '/Lockin_offset'
    ...
    [photons, bias, temp, mod_volt,
     ...
     carrier_power, mod_freq] = parameters
    ...
    x_noise_mean = np.mean(data_off_res_0_off[1][:-points_omit])
    y_noise_mean = np.mean(data_off_res_0_off[2][:-points_omit])
    x_noise_err = np.std(data_off_res_0_off[1][:-points_omit])
    ...
    plt.title('Mean X and Y = {}, {} mV\n and STD = {}, {} mV'.format(
            round(x_noise_mean*1e3,2), round(y_noise_mean*1e3,2),
            round(x_noise_err*1e3,2), 
    ...
    filename = directory_name + '/X_Quadrature_Setup'
    plt.savefig(filename+'.jpeg')
    plt.close()
    ...
    xf_off_res_noise = fft(data_off_res_1_off[1][:-points_omit])
    yf_off_res_noise = fft(data_off_res_1_off[2][:-points_omit])
    xf_on_res_noise = fft(data_on_res_0_sig[1][:-points_omit])
    yf_on_res_noise = fft(data_on_res_0_sig[2][:-points_omit])
    xf_on_res_sig = fft(data_on_res_1_sig[1][:-points_omit])
    yf_on_res_sig = fft(data_on_res_1_sig[2][:-points_omit])
    valid_off_res_x = 2/N*np.abs(xf_off_res_noise[:N//2])
    valid_on_res_x = 2/N*np.abs(xf_on_res_noise[:N//2])
    valid_sig_x = 2/N*np.abs(xf_on_res_sig[:N//2])
    ...
    plt.plot(tf[1:], valid_off_res_x[1:],
             label = 'off resonance noise', lw=4)
    plt.plot(tf[1:], valid_on_res_x[1:],
             label = 'on resonance noise', lw=4)
    plt.plot(tf[1:], valid_sig_x[1:],
...
#                   np.max(valid_sig_x)])
#    plt.ylim(top = ylim_x)
    filename = directory_name + '/X_Quadrature_Setup_FFT'
    plt.savefig(filename+'.jpeg')
    plt.close()
...
#    plt.ylim(top = ylim_y)
    ...
    np.savetxt(directory_name_off +'/fft.dat', np.column_stack((
            tf, valid_off_res_x, valid_off_res_y,
            valid_on_res_x, valid_on_res_y,
            valid_sig_x, valid_sig_y)))
