...
directory_name=('../{}_{}_{}_{}_long_measurement_{}_mK_{}_ng_{}_phi0_{}_photons'.
...
if feed_bool:
    feed_prompt = input('''Continue to feedback measurement? (y/n)''')
    if feed_prompt=='y': #prompts to check the feedback loop to continue.
        count = count+1
        print('Continuing measurements..')
        PID_prompt = input(''' Open alazar software, switch on/off feedback 
                           and start
                           acquisition. Open Parameters file and add PID values.
                           Enter y after measurement.''')
        if PID_prompt == 'y':
            print('Measurements completed.')
        else:
            print('No long measurement done.')
